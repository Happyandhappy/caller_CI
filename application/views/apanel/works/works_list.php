<div class="row-fluid sortable">		
			  ...
			  <?php } ?>
					...
					<div class="box-content">
                     <a href="<?php echo base_url();?>apanel/works_add/">
		...
		<table id="data-table" class="table table-striped table-bordered nowrap">
          ...
          <tbody>
            <?php
        $this->db->order_by('works_id', 'asc');
        $test = $this->db->get('tbl_howitworks')->result_array();
        $caounter=0;
        foreach ($test as $row):$caounter++;?>
            <tr>
              <td style="width:30px;"><?php echo $caounter;?></td>
              <td><img src="<?php echo base_url();?>/uploads/works_img/<?php echo $row['image']; ?>" alt="..."></td>
              <td><?php echo $row['title']; ?></td>
              <td><?php if($row['status']=='1'){ echo "Active"; }
				  else { echo "Deleted"; }				  
				  ?></td>
              <td style="width:200px"><a href="<?php echo base_url().'apanel/works_edit/'.$row['works_id']; ?>">
                <button class="btn btn-primary"> Update</button>
                </a>
				<a href="<?php echo base_url().'apanel/works/delete/'. $row['works_id']; ?>">
