...
<form name="msform" id="msform" novalidate="novalidate" class="box">
 ...
 <?php } ?>
  ...
  <div class="form-group row">
    <div class="col-sm-8 col-md-8">
      ...
      <a class="button btn-medium sky-blue1">Send Pin</a> </div>
    <div class="col-sm-8 col-md-8">
      <label>Verify</label>
      <input type="text" name="pin" id="pin" class="input-text full-width" value="" placeholder="Enter the Pin" />
      <a class="button btn-medium sky-blue1">Verify</a> </div>
  </div>
  <hr />
  <div class="form-group row">
    <div class="">
 ...
 <?php } ?>
 <?php if($this->session->userdata('user_id')=="") { ?>
  <input type="button" name="next_business_form" id="next_business_form" class="next button btn-medium green" value="Submit My Job" />
   <?php } else {?>
  <input type="button" name="next_business_form" id="next_business_form" class="next button btn-medium green" value="Submit My Job" />
 <?php } ?>
</div>
    ...
    $("#msform").validate({
        ...
        rules: {
			...
			email: {
                ...
                email: true,
                remote: {
                    url: "<?php echo base_url();?>index.php?trades/email_check",
                    type: "post"
                }
			},	
			password :"required",
...
$(".check").click(function(){
  ...
  if($('#user_email').valid() && $('#user_password').valid()){
		...
		var postData={
			...
			$.ajax({
			url: "<?php echo base_url();?>index.php?trades/user_login",
			type: "POST",  
			data: postData,
			success: function(data){
			//alert(data);
			if(data=='Enter Correct Details'){
			$("#msg").append('<p> Enter Correct Details</p>');
				return false;
			}		
			else{
				//alert('true');
...
$(".next").click(function(){
  ...
  if($('#zipcode').valid() && $('#phoneno').valid() && $('#pin').valid()){
		...
		var postData={
			...
			$.ajax({
			url: "<?php echo base_url();?>index.php?trades/add_trades",
			type: "POST",  
			data: postData,
			success: function(data){
			 $('#details').html(data);
...
$(".first").click(function(){
	...
	function login_trades()
	{
		...
		var postData={
			...
			$.ajax({
			url: "<?php echo base_url();?>index.php?trades/login_trades",
			type: "POST",  
			data: postData,
			success: function(data){
			//alert(data);
